...
async function loadMatches(candidateId) {
  const matches = await getCandidateMatches(candidateId);
...
function getMatchActionButtons(match) {
  // Candidates can only view - no action buttons needed
  // They can click the card to open details and send messages
  return '';
}
...
async function sendMessage() {
  ...
  const message = {
    ...
    recipientId: String(match.companyId),
